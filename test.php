<?php
/**
 * Quick diagnostic page for WAMP setup
 * Access at: http://localhost/PA/test.php
 */
...
<body>
    <h1>🐦 Budgie - Test de Configuration WAMP</h1>
    <p>Cette page vérifie que votre installation est correcte.</p>
    ...
    echo '<div class="test ' . (file_exists($configFile) ? 'success' : 'error') . '">';
    echo '<strong>' . (file_exists($configFile) ? '✓' : '✗') . ' Fichier config.php:</strong> ';
    if (file_exists($configFile)) {
        echo '<code>' . $configFile . '</code>';
        $successes[] = 'Fichier config trouvé';
    } else {
        echo 'Fichier manquant !';
        $errors[] = 'config.php introuvable';
    }
    echo '</div>';
    ...
    try {
        ...
        $db = getDB();
        echo 'success">';
        echo '<strong>✓ Connexion Base de Données:</strong> Réussie';
        echo '<br><small>Host: ' . DB_HOST . ' | Database: ' . DB_NAME . ' | User: ' . DB_USER . '</small>';
        $successes[] = 'Connexion DB OK';
        
        // Test 5: Check tables
        echo '</div><div class="test ';
        $tables = ['users', 'accounts', 'expenses', 'incomes', 'exceptions'];
        $missingTables = [];
        ...
        if (empty($missingTables)) {
            echo 'success">';
            echo '<strong>✓ Tables de la Base:</strong> Toutes présentes (' . count($tables) . ' tables)';
        ...
        echo 'error">';
        echo '<strong>✗ Connexion Base de Données:</strong> Échec';
        echo '<br><span style="color: #ef4444;">Erreur: ' . htmlspecialchars($e->getMessage()) . '</span>';
        echo '<br><br><strong>Solutions possibles:</strong>';
        echo '<ol>';
        echo '<li>Vérifiez que MySQL est démarré dans WAMP (icône verte)</li>';
        echo '<li>Créez la base de données <code>budgie_db</code> dans phpMyAdmin</li>';
        echo '<li>Vérifiez les identifiants dans <code>src/config/config.php</code></li>';
        echo '</ol>';
        $errors[] = 'Connexion DB échouée';
    }
    echo '</div>';
    ...
    if (empty($errors) && count($warnings) <= 1) {
        ...
        echo '<li><a href="public/login.php" style="color: #2563eb;">Page de connexion</a></li>';
        echo '<li><a href="public/signup.php" style="color: #2563eb;">Page d\'inscription</a></li>';
        echo '</ul>';
    } else {
        echo '<hr>';
    ...
    <div style="background: #f1f5f9; padding: 15px; margin-top: 20px; border-radius: 8px; font-size: 0.9em;">
        ...
        <ul>
            ...
            <li><a href="README.md">Documentation complète</a></li>
        </ul>
    </div>
</body>
</html>
